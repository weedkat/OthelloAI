...
class Menu:
    def __init__(self):
        ...
        self.agents = ["Genetic Algorithm", "Minimax", "Genetic Algorithm Adaptive"]
        self.submenu_mode = 0
        self.agent_a = None
    ...
    def draw_submenu(self, menu = 0):
        """
        Draw the submenu on the Pygame window.
        """
        self.win.blit(self.background_image, (0, 0))  # Draw the background image
        submenu_items = self.agents if menu else self.submenu_items 
        buttons = []
        num_submenu_items = len(submenu_items)
                        ...
                        for button in buttons:
                            if button.check_collision((x, y)):
                                if button.text == "Multi-player\n(Play with Friend)":
                                    ...
                                    self.draw_submenu(self.submenu_mode)
                                
                                elif button.text == "Ai Agents\n(Agent vs Agent)":
                                    # Pass the draw_menu function as a callback to return to the main menu
                                    self.submenu_mode = 1
                                    self.draw_submenu(self.submenu_mode)
                                ...
                                for agent in self.agents:
                                    if button.text == agent:
                                        if self.play_with_ai:     
                                            othello_gui = OthelloGUI(player_mode="ai", agent_a = agent)
                                            othello_gui.run_game(
                                                return_to_menu_callback=self.draw_menu
                                            )
                                        elif self.agent_a:
                                            othello_gui = OthelloGUI(player_mode="agent", agent_a = self.agent_a, agent_b = agent)
                                            self.agent_a = None
                                            othello_gui.run_game(
                                                return_to_menu_callback=self.draw_menu
                                            )
